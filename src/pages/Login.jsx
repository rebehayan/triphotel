...
const Login = ({ close, ...props }) => {
    ...
    try {
      ...
      if (error.response) {
        switch (error.response.status) {
          case 401:
            setLoginError("이메일 혹은 비밀번호가 정확하지 않습니다. 다시 입력해 주세요.");
            break;
          default:
            setLoginError("로그인 중 예기치 않은 오류가 발생했습니다.");
        }
      } else {
        setLoginError("서버에 접속할 수 없습니다. 네트워크 상태를 확인해 주세요.");
      }
      setLoginToast(true);
  ...
  const handleRegister = async (e) => {
    ...
    const monthValid = birthMonth >= 1 && birthMonth <= 12 && /^\d{2}$/.test(birthMonth);
    if (!monthValid) {
    ...
    const dayValid = birthDay >= 1 && birthDay <= 31 && /^\d{2}$/.test(birthDay);
    if (!dayValid) {
    ...
    try {
      ...
      if (error.response) {
        ...
        setRegisterError("서버에 접속할 수 없습니다. 네트워크 상태를 확인해 주세요.");
      }
      setRegisterToast(true);
      ...
      {isTab === "login" && (
        <>
          <form className="login-form relative" onSubmit={handleLogin}>
            ...
            <div>
              이메일
              <Input type="email" required value={loginEmail} onChange={handleLoginEmail} />
            </div>
            <div>
              비밀번호
              <Input type="password" required value={loginPassword} onChange={handleLoginPassword} />
            </div>
          ...
          <Toast color={"red"} onOpen={loginToast} onClose={() => setLoginToast(false)}>
            {loginError}
          </Toast>
        </>
      )}
      {isTab === "register" && (
        <>
          <form className="login-form relative" onSubmit={handleRegister}>
            ...
            <div>
              이메일
              <Input type="email" required value={registerEmail} onChange={handleRegisterEmail} />
            </div>
            <div>
              생년월일
              <div className="birth">
                <Input type="number" placeholder="YYYY" required value={birthYear} onChange={handleBirthYear} />
                년
                <Input type="number" placeholder="MM" required value={birthMonth} onChange={handleBirthMonth} />
                월
                <Input type="number" placeholder="DD" required value={birthDay} onChange={handleBirthDay} />일
              </div>
            </div>
            <div>
              비밀번호
              <Input type="password" required value={registerPassword} onChange={handleRegisterPassword} />
            </div>
            <div>
              비밀번호 확인
              <Input type="password" required value={confirmPassword} onChange={handleConfirmPassword} />
            </div>
            <button type="submit" className="btn-blue xl2">
              Sign Up
            </button>
