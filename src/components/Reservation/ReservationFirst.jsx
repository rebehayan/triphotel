...
const ReservationFirst = () => {
  ...
  useEffect(() => {
    ...
    const handleCalculate = () => {
      const countMember =
        parseInt(isPayInfo.adult_count) + parseInt(isPayInfo.child_count);
      const adult_fare =
        parseInt(isPayInfo.adult_count) * reservedRoom.adult_fare;
      const childrenPay =
        parseInt(isPayInfo.child_count) * reservedRoom.child_fare;
      ...
      if (reservedRoom.maximum_capacity < countMember) {
        ...
        reservedRoom.standard_capacity < countMember &&
        reservedRoom.maximum_capacity >= countMember
      ) {
        setIsToast2(true);
        setIsPayInfo({ ...isPayInfo, adult_fare, childrenPay, total_price });
      } else {
    ...
    isPayInfo.adult_count,
    isPayInfo.child_count,
    isPayInfo.check_in,
    isPayInfo.check_out,
  ]);
    ...
    <>
        ...
        <form className="reservation-write" onSubmit={handleSubmit}>
          ...
          >
            ...
            <li>
              ...
              <GuestCounter
                iscount={handleAdult}
                max={reservedRoom.maximum_capacity}
              />
            </li>
            <li>
              ...
              <GuestCounter
                kids
                iscount={handleChildren}
                max={reservedRoom.maximum_capacity}
              />
            </li>
            <li className="!grid grid-cols-2">
              <strong className="--title">
                성인 ⨉ {isPayInfo.adult_count ? isPayInfo.adult_count : 0}
              </strong>
              <span className="--total justify-self-end">
                ₩ {isPayInfo.adult_fare ? digit3(isPayInfo.adult_fare) : 0}
              </span>
              <strong className="--title">
                어린이 ⨉ {isPayInfo.child_count ? isPayInfo.child_count : 0}
              </strong>
              <span className="--total justify-self-end">
            ...
            {isWidth < 1024 && (
              ...
              >
                <MdKeyboardDoubleArrowUp
                  className={`ico-toggle ${!isToggle ? "active" : ""}`}
                />
              </button>
            )}
            <button
      ...
      {isToast && (
        <Toast onOpen={isToast} onClose={() => setIsToast(false)} color={"red"}>
          인원수가 초과되었습니다. 최대 {reservedRoom.maximum_capacity}명 수용
          가능합니다.
        </Toast>
      )}
      {isToast2 && (
        <Toast
          ...
          color={"blue"}
        >
          해당 숙소는 {reservedRoom.standard_capacity + 1}인이상{" "}
          {reservedRoom.maximum_capacity}이하면 추가금액이 발생합니다.
        </Toast>
      )}
